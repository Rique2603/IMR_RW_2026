...
def main():
    ...
    pygame.display.set_caption("Aula 02: Controle Proporcional para um Ponto")
    ...
    while running:
        ...
        if target is not None:
            ...
            angle_error = math.atan2(math.sin(desired_theta - robot.theta), math.cos(desired_theta - robot.theta))
            ...
            if distance > stop_radius:
                if abs(angle_error) > 0.08:
                    ...
                    omega_cmd = 0.0
                    v_cmd = min(max_linear_speed, kp_linear * distance)
            else:
                v_cmd = 0.0
                omega_cmd = 0.0
        ...
        if target is not None:
            pygame.draw.circle(screen, (255, 120, 120), (int(target[0]), int(target[1])), 7)
            pygame.draw.circle(screen, (255, 180, 180), (int(target[0]), int(target[1])), 12, 2)

        target_label = "Nenhum alvo" if target is None else f"Alvo: ({target[0]:.0f}, {target[1]:.0f})"
        info_txt = [
            f"Pose X: {robot.x:.1f} px | Y: {robot.y:.1f} px | Theta: {math.degrees(robot.theta):.1f} deg",
            f"Target: {target_label} | v = {robot.v:.1f} px/s | omega = {robot.omega:.2f} rad/s",
            "Clique na tela para definir um ponto-alvo. O robô vai se alinhar e avançar até ele.",
            "Parada automática: distância <= 10 px"
        ]
