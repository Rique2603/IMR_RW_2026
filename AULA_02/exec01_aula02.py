...
def main():
    ...
    while running:
        ...
        # Leitura do teclado para controle das velocidades das rodas
        keys = pygame.key.get_pressed()
        v_left = 0.0
        v_right = 0.0

        if keys[pygame.K_w]:
            v_left += 120.0    # roda esquerda para frente
        if keys[pygame.K_s]:
            v_left -= 80.0     # roda esquerda para trás
        if keys[pygame.K_i]:
            v_right += 120.0   # roda direita para frente
        if keys[pygame.K_k]:
            v_right -= 80.0    # roda direita para trás

        # Converte velocidades das rodas em v e omega do robô
        ...
        info_txt = [
            ...
            f"Comandos: v = {robot.v:.1f} px/s | omega = {robot.omega:.2f} rad/s | rodas = ({v_left:.1f}, {v_right:.1f}) px/s",
